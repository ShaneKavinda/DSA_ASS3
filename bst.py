...
def rearrange_for_balanced_BST(arr, start, end):
    if start > end:
        return None
    ...
    root.left = rearrange_for_balanced_BST(arr, start, mid - 1)
    root.right = rearrange_for_balanced_BST(arr, mid + 1, end)
...
def complete_bst_from_balanced_bst(root):
    sorted_nodes = []
    in_order_traversal(root, sorted_nodes)

    n = len(sorted_nodes)
    return rearrange_for_balanced_BST(sorted_nodes, 0, n - 1)
...
def main():
    ...
    # Task 1
    balanced_BST = rearrange_for_balanced_BST(sorted(original_sequence), 0, len(original_sequence)-1)
    print("Original Data Sequence:", original_sequence)

    # Print the tree shape of the BST
    printTree(balanced_BST)

    # Convert the balanced BST to a complete BST
    complete_bst_root = complete_bst_from_balanced_bst(balanced_BST)

    # Print the tree shape of the BST
    printTree(complete_bst_root)
